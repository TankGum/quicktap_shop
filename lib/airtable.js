...
// nếu người điền chỉ gõ toàn chữ số thì tự format thành "199.000 VND"; còn nếu họ đã ghi rõ
// kiểu "199.000đ/cái" hay "Liên hệ" thì giữ nguyên phần chữ, chỉ đổi ký hiệu tiền.
//
// Đơn vị hiển thị TOÀN SITE là "VND". Vì người điền Airtable vẫn quen gõ "đ"/"₫"/"VNĐ", phải
// chuẩn hoá luôn ở đây — đây là chỗ duy nhất sinh ra chuỗi giá, sửa một chỗ là mọi nơi hiển
// thị đổi theo.
//
// Điều kiện "đứng ngay sau chữ số" là bắt buộc, không được thay "đ" ở khắp nơi: "Liên hệ để
// biết giá" cũng có chữ "đ", thay mù sẽ ra "Liên hệ VND biết giá".
function formatPrice(price) {
  if (price === undefined || price === null || price === '') return '';
  const asNumber = Number(price);
  if (/^\d+$/.test(String(price).trim())) return `${asNumber.toLocaleString('vi-VN')} VND`;
  return String(price)
    .replace(/(\d)\s*[đ₫]/g, '$1 VND')
    .replace(/(\d)\s*vn[đd]/gi, '$1 VND');
}
...
//   Key  (Single line text) — heroVideo | heroVideoMobile | heroImage | demoVideo
//                             (xem MEDIA_KEYS bên dưới)
...
//
// heroVideoMobile là bản DỌC (9:16) của heroVideo cho điện thoại — không bắt buộc; thiếu thì
// điện thoại cũng chiếu bản ngang như trước.
//
// Trả về { heroVideo: {url, alt} | null, heroVideoMobile: ..., heroImage: ..., demoVideo: ... }
// — ô nào chưa có dòng/chưa có file thì là null, trang chủ tự lùi về phương án dự phòng (xem
// app/page.js).
const MEDIA_KEYS = {
  herovideo: 'heroVideo',
...
export const getSiteMedia = cache(async function getSiteMedia() {
  ...
  if (!ok) {
    return fallbackToStaleCache(SITE_MEDIA_CACHE_FILE, 'media trang chủ', {
      heroVideo: null, heroVideoMobile: null, heroImage: null, demoVideo: null,
    });
  }
  ...
  const media = { heroVideo: null, heroVideoMobile: null, heroImage: null, demoVideo: null };
  for (const item of processed) {
