 ...
 */
// Trạng thái trong D1 là tiếng Anh (snake_case) cho SQL đọc; model thì cần câu tiếng Việt nói
// đúng ý — nhất là 'pending_payment', vì trả lời sai chỗ này là hoặc giục nhầm người đã trả
// tiền, hoặc xác nhận nhầm là đã nhận được tiền.
const STATUS_TEXT = {
  pending_payment: 'đã tạo đơn, CHƯA thấy tiền chuyển về',
  pending_confirmation: 'đã ghi nhận, đang chờ nhân viên gọi xác nhận',
...
function orderSection(order) {
  ...
  // Đơn từ giỏ hàng: nhiều dòng hàng, có tiền và có trạng thái thanh toán. Đây là những thứ
  // khách hỏi nhiều nhất sau khi đặt ("chuyển tiền chưa", "hết bao nhiêu"), nên phải có mặt —
  // nhưng vẫn KHÔNG có tên, số điện thoại hay địa chỉ (lookupOrder cố tình không SELECT chúng).
  const lines =
    order.kind === 'shop'
      ? [
          `- Mã đơn: ${order.code}`,
          `- Ngày đặt: ${when}`,
          ...(order.lines || []).map(
            (l) => `- ${l.name} × ${l.quantity} (${money(l.unitPrice)}/cái)`
          ),
          order.discount > 0 ? `- Đã giảm: ${money(order.discount)}` : null,
          `- Phí giao: ${order.shippingFee === 0 ? 'miễn phí' : money(order.shippingFee)}`,
          `- Tổng tiền: ${money(order.total)}`,
          `- Thanh toán: ${order.paymentMethod === 'cod' ? 'khi nhận hàng (COD)' : 'chuyển khoản'}`,
...
export function buildSystemPrompt(kb, order) {
  ...
  return [
    ...
    '- Không bao giờ đọc lại tên quán, số điện thoại hay ĐỊA CHỈ của khách từ dữ liệu đơn.',
